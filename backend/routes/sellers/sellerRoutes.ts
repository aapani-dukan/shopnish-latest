        ...
        try {
            ...
            await db.transaction(async (tx) => {
                ...
                await tx.insert(orderTracking).values({
                    ...
                    status: finalStatusForSubOrder,
                    updatedByUserId: userId, // सेलर का यूजर ID
                    updatedByUserRole: userRoleEnum.enumValues[1], // 'seller'
                    timestamp: new Date(),
                ...
                if (allSubOrdersReadyForDeliveryOrSelfDelivered) {
                    ...
                    if (currentMasterOrder && currentMasterOrder.status !== masterOrderStatusEnum.enumValues[2] /* processing */) {
                        await tx.update(orders)
                            .set({ status: masterOrderStatusEnum.enumValues[2], updatedAt: new Date() }) // 'processing'
                        ...
                        await tx.insert(orderTracking).values({
                            ...
                            updatedByUserRole: userRoleEnum.enumValues[1],
                            timestamp: new Date(),
